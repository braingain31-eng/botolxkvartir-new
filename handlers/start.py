...
async def search_text(call: CallbackQuery):
    text = (
        ...
        "<i>Пример:</i> _Вилла на 4 человека, до 3000 ₿/ночь, в Анжуне, с 15 декабря на неделю, с бассейном_"
    )

    # Отправляем как обычное сообщение — пользователь увидит сразу
...
async def search_voice(call: CallbackQuery):
    ...
    if premium_info["is_premium"]:
        # Премиум — разрешаем голосовой поиск
        await call.message.answer(
            "Голосовой поиск активирован!\n\n"
            "Говори запрос — я пойму всё:\n"
            "• Тип жилья\n"
            "• Бюджет\n"
            "• Район\n"
            "• Даты\n"
            "• Кол-во человек\n"
            "• Пожелания\n\n"
            "Нажми и держи микрофон — и говори!",
            parse_mode="HTML"
        )
    else:
        # Не премиум — показываем преимущества и меню оплаты
        text = (
            "<b>Поиск голосом</b>\n\n"
            "Голосовой поиск доступен только с премиум-подпиской!\n\n"
            "Премиум даёт:\n"
            "• Все контакты хозяев — бронь напрямую\n"
...
• Контакты хозяев
• Приоритет в поиске
• Чтобы бот сам присылал тебе новые квартиры первому
